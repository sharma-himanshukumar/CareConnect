...
def refine_prompt_call_LLM(raw_query: str) -> str:
    ...
    system_message = (
        ...
        "Example:\n"
        "Input: 'Hi, I need some help, suggest me some names for XYZ problem'\n"
        "Output: Medicine names based on the XYZ problem.\n\n"
    )
...
class DatabricksVectorDBManager:
    ...
    def __init__(self,
        ...
        self.index_name = f"{catalog_name}.{schema_name}.{index_table_name}"
...
def call_vector_db(api_key, query, top_n):
        query = refine_prompt_call_LLM(query)
        embeddings = Embeddings(api_key).get_embeddings(query)
        ENDPOINT_NAME = "careconnect_endpoints" 
        CATALOG_NAME = "careconnect" 
        SCHEMA_NAME = "default"
        INDEX_TABLE_NAME = "careconnect_medical_prescription_vector_db_idx"
        EMBEDDING_DIM = 1024
        db_manager = DatabricksVectorDBManager(
                endpoint_name=ENDPOINT_NAME,
                catalog_name=CATALOG_NAME,
                ...
                index_table_name=INDEX_TABLE_NAME,
                embedding_dimension=EMBEDDING_DIM,
                embedding_vector_column="__db_indication_vector",
                index_schema={
                "id": "int",
                "name": "string",
                "category": "string",
                ...
                "side_effect": "string",
                "rule": "string",
                "usage_periode": "string",
                "indication": "string",
                "__db_indication_vector": f"array<float>"
        ...
        if db_manager.index:
            ...
            num_results=top_n,
            columns=["name","category","indication","price","composition","drug_interaction","contra_indication","dose","side_effect","rule","usage_periode"]
            )
...
if __name__ == "__main__":
    ...
    call_vector_db(api_key,"fever", 5)
    call_vector_db(api_key,"headache and cough", 5)
